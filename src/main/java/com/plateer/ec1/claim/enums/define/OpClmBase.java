...
public enum OpClmBase {
    ...
    GCC(OPT0003Code.CANCEL.code, OPT0004Code.CANCEL_COMPLETE.code,
            toList(OPT0013Code.DELIVERY.code), IntUnaryOperator.identity(), Boolean.TRUE, ClaimStatusType.COMPLETE),
    //모바일쿠폰주문취소접수
    MCA(OPT0003Code.CANCEL.code, OPT0004Code.CANCEL_REQUEST.code,
            toList(OPT0013Code.DELIVERY.code), IntUnaryOperator.identity(), Boolean.TRUE, ClaimStatusType.ACCEPT_WITHDRAWAL),
    //모바일쿠폰주문취소완료
    MCC(OPT0003Code.CANCEL.code, OPT0004Code.CANCEL_COMPLETE.code,
            toList(OPT0013Code.DELIVERY.code), IntUnaryOperator.identity(), Boolean.FALSE, ClaimStatusType.COMPLETE),

    //반품접수
    RA(OPT0003Code.RETURN_ACCEPT.code, OPT0004Code.RETURN_ACCEPT.code,
            toList(OPT0013Code.RETRIEVE.code), (dvp) -> dvp+1, Boolean.TRUE, ClaimStatusType.ACCEPT_WITHDRAWAL),
    //반품철회
    RW(OPT0003Code.RETURN_WITHDRAWAL.code, OPT0004Code.RETURN_WITHDRAWAL.code,
            toList(OPT0013Code.RETRIEVE.code), IntUnaryOperator.identity(), Boolean.TRUE, ClaimStatusType.ACCEPT_WITHDRAWAL),

    //교환접수
    EA(OPT0003Code.EXCHANGE_ACCEPT.code, OPT0004Code.EXCHANGE_ACCEPT.code,
            toList(OPT0013Code.DELIVERY.code, OPT0013Code.RETRIEVE.code), (dvp) -> dvp+1,  Boolean.TRUE, ClaimStatusType.ACCEPT_WITHDRAWAL),
    //교환철회
    EW(OPT0003Code.EXCHANGE_WITHDRAWAL.code, OPT0004Code.EXCHANGE_WITHDRAWAL.code,
            toList(OPT0013Code.DELIVERY.code, OPT0013Code.RETRIEVE.code), IntUnaryOperator.identity(), Boolean.TRUE, ClaimStatusType.ACCEPT_WITHDRAWAL);
